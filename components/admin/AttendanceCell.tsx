...
function formatMarkedAtTime(markedAt: string): string {
...
export function AttendanceCell({
  ...
  const presenceTitle = checked
    ? `${markedManually ? "Marcada manualmente por el docente" : "Marcada con el código de asistencia"}${markedAt ? ` a las ${formatMarkedAtTime(markedAt)}` : ""}`
    : undefined;
  const title = status === "error" ? "No se pudo guardar. Intenta de nuevo." : presenceTitle;
