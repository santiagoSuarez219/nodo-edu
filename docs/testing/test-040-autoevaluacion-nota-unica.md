...
- Dos casos UI aprobados con reservas: TC-013/TC-014 (renombrado imposible por bug real, DEBT-046); TC-015 (hallazgo menor de refresco de UI tras recalcular, no bloqueante).
...
- Casos dependientes de spec-039: TC-021 marcado **No aplica** — spec-039 está `[DONE]` y mergeado a `development`, pero esta rama no incluye el cableado del filtro BLOQUE 039 (creada antes del merge). Deuda ya registrada como DEBT-045.
- Casos sin UI, ejecutados como verificación asistida por API/base de datos: TC-017 (paso 3), TC-018, TC-019.
- Hallazgos escalados a `docs/specs/backlog.md`: **DEBT-046** (no existe forma de renombrar un ítem de calificación — bug real, `GradeItemsPanel.tsx` sin control de renombrado pese a que la UI lo promete, y la Server Action correspondiente no está cableada a ningún componente).
- Limpieza de datos de prueba: ✅ Completada (2026-08-02)
  - Orden ejecutado: 42 preguntas (`question-bank-mcp` → `delete_question`, incluidas P31, P32 y las 5 de L8) → 6 matrículas (`unenroll_student`, E5 en sus dos cursos) → 6 estudiantes E1–E6 (`delete_student`, sin `409` en ninguno) → docente D2 (Supabase Auth Admin API `DELETE /auth/v1/admin/users/{id}`, cascadeó su fila de `user_roles`, verificado en 0) → cursos académicos Grupo A y Grupo B (`delete` directo vía `service_role`, único camino posible ya que se crearon por el mismo mecanismo). Verificado tras el borrado: 0 filas huérfanas en `grade_items` y `enrollments` para ambos cursos.
  - ⚠️ **Residuo permanente esperado, no un huérfano accidental:** las 2 filas de `self_assessment_attempts_discarded` sembradas en TC-019 para `(E2, sintaxis-de-python)` siguen en la base tras borrar a E2 — esa tabla (`create table ... like self_assessment_attempts including defaults`) no copió las foreign keys, así que no cascadea con el borrado del usuario de Auth. Sin endpoint de borrado por diseño (intento único, sin política RLS de `delete`); quedan ahí documentadas para no confundirlas con datos reales de producción.
  - El `grade_item` `kind='self_assessment'` de cada curso académico (no borrable por separado, TC-014) desapareció correctamente al eliminar Grupo A y Grupo B.
  - Nota operativa de esta ronda: TC-019 quitó y volvió a crear el constraint único `self_assessment_attempts_one_per_lesson` en la base de desarrollo — verificado restaurado y funcionando (bloqueó un insert de prueba con `23505`).
