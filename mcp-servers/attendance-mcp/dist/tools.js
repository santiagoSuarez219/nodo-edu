...
export const tools = [
    {
        name: "list_sessions",
        description: "Lista sesiones de asistencia de un curso con filtros opcionales. Útil para revisar el histórico de sesiones abiertas.",
        inputSchema: {
            type: "object",
            properties: {
                course_id: {
    ...
    },
    {
        ...
        description: "Obtiene el roster de asistencia completo de una sesión específica con nombres de estudiantes y horas de marcado.",
        inputSchema: {
            type: "object",
            properties: {
                session_id: {
    ...
    },
    {
        ...
        description: "Obtiene un resumen de asistencia por estudiante para un curso con porcentajes de asistencia y sesiones asistidas.",
        inputSchema: {
            type: "object",
            properties: {
                course_id: {
...
export async function processToolCall(toolName, toolInput) {
    switch (toolName) {
        ...
        case "get_session_attendance":
            return await callAttendanceApi("GET", `/attendance/sessions/${toolInput.session_id}/records`);
        case "get_course_attendance_summary":
            return await callAttendanceApi("GET", `/attendance/courses/${toolInput.course_id}/summary`, {
                ...(toolInput.from_date ? { fromDate: String(toolInput.from_date) } : {}),
                ...(toolInput.to_date ? { toDate: String(toolInput.to_date) } : {}),
            });
        default:
