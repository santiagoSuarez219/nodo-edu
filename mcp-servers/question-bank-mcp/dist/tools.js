...
export const tools = [
    {
        name: "list_questions",
        description: "Lista preguntas del banco con filtros opcionales. Úsala para explorar el banco antes de crear y evitar duplicados. course_slug/lesson_slug filtran por preguntas MONTADAS en esa lección (ver list_lesson_questions para el orden exacto).",
        inputSchema: {
            type: "object",
            properties: {
                type: {
    ...
    {
        name: "get_question",
        description: "Obtiene una pregunta completa por ID (con choices/rubric/challenge_tests, keywords y las lecciones donde está montada). Úsala antes de actualizar.",
        inputSchema: {
            type: "object",
            properties: {
                id: {
        ...
        inputSchema: {
            ...
            properties: {
                ...
                keywords: {
                    type: "array",
                    items: { type: "string" },
                    description: "Slugs de keywords del catálogo (NUNCA inventar una: listar con list_keywords primero, o proponer create_keyword al usuario).",
                },
                topic_title: {
                    type: "string",
    ...
    },
    {
        ...
        description: "Actualiza una pregunta (envía campos a cambiar; el MCP relee y compone el payload completo). No cambia el type y NO afecta en qué lecciones está montada (usa mount_question_in_lesson/unmount_question_from_lesson para eso).",
        inputSchema: {
            type: "object",
            properties: {
                id: {
    ...
    },
    {
        ...
        description: "Publica una pregunta (is_published=true). La API valida ≥1 opción correcta en multiple_choice. Publicarla NO la monta en ninguna lección — sigue siendo invisible en autoevaluaciones hasta mount_question_in_lesson.",
        inputSchema: {
            type: "object",
            properties: {
                id: {
    ...
    {
        name: "list_keywords",
        description: "Lista el catálogo de keywords (vocabulario controlado). Úsala SIEMPRE antes de asignar keywords a una pregunta — nunca inventar un slug sin confirmarlo aquí.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    {
        name: "create_keyword",
        description: "Crea una keyword nueva en el catálogo compartido. Amplía un vocabulario que usan todas las lecciones y cursos: PROPONLA AL USUARIO antes de invocarla, no la crees de forma unilateral solo porque una pregunta la necesita. Falla con 409 si el slug ya existe.",
        inputSchema: {
            type: "object",
            properties: {
                slug: {
    ...
    // ─── Montaje de preguntas en lecciones (spec-042) ───────────────────────
    {
        ...
        description: "Monta una pregunta publicada en una lección: la hace aparecer en la autoevaluación de esa lección. Idempotente (montarla dos veces no la mueve de posición). Una pregunta puede montarse en varias lecciones a la vez.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    },
    {
        ...
        description: "Desmonta una pregunta de una lección: deja de aparecer en la autoevaluación de esa lección, sin eliminarla del banco ni afectar sus otros montajes.",
        inputSchema: {
            type: "object",
            properties: {
                question_id: { type: "string", description: "UUID de la pregunta" },
    ...
    },
    {
        ...
        description: "Lista las preguntas montadas en una lección, EN EL ORDEN en que las verá el estudiante en su autoevaluación. Úsala para verificar que una pregunta publicada quedó realmente visible.",
        inputSchema: {
            type: "object",
            properties: {
                course_slug: { type: "string", description: "Slug del curso" },
    ...
    },
    {
        ...
        description: "Reordena las preguntas YA MONTADAS en una lección. La lista de question_ids debe incluir EXACTAMENTE las preguntas montadas (ni de más ni de menos) o falla sin cambiar nada — este endpoint nunca monta ni desmonta, solo reordena.",
        inputSchema: {
            type: "object",
            properties: {
                course_slug: { type: "string", description: "Slug del curso" },
...
async function handleListQuestions(input) {
    const params = new URLSearchParams();
    if (input.course_slug)
        params.append("course_slug", String(input.course_slug));
    if (input.lesson_slug)
        params.append("lesson_slug", String(input.lesson_slug));
    if (input.type)
        params.append("type", String(input.type));
    if (input.difficulty)
        params.append("difficulty", String(input.difficulty));
    if (input.is_published !== undefined)
        params.append("is_published", String(input.is_published));
    if (input.keyword)
        params.append("keyword", String(input.keyword));
    if (input.q)
        params.append("q", String(input.q));
    if (input.limit)
...
async function handleGetQuestion(input) {
...
async function handleUpdateQuestion(input) {
    const id = String(input.id);
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    const { id: _id, ...body } = input;
    const current = (await callQuestionBankApi("GET", `/${id}`));
    ...
    const filtered = Object.fromEntries(Object.entries(current.data).filter(([key, value]) => allowedFields.includes(key) && value !== null));
    const merged = {
        ...filtered,
        ...body,
    };
...
async function handleDeleteQuestion(input) {
    const id = String(input.id);
    const result = await callQuestionBankApi("DELETE", `/${id}`);
    return JSON.stringify(result, null, 2);
}
async function handlePublishQuestion(input) {
    const id = String(input.id);
    const result = await callQuestionBankApi("POST", `/${id}/publish`);
    return JSON.stringify(result, null, 2);
}
async function handleListKeywords(input) {
    const params = new URLSearchParams();
    if (input.q)
        params.append("q", String(input.q));
    if (input.kind)
        params.append("kind", String(input.kind));
    if (input.limit)
        params.append("limit", String(input.limit));
    if (input.offset !== undefined)
        params.append("offset", String(input.offset));
...
async function handleCreateKeyword(input) {
    const result = await callKeywordsApi("POST", "", input);
    return JSON.stringify(result, null, 2);
}
async function handleMountQuestionInLesson(input) {
...
async function handleUnmountQuestionFromLesson(input) {
    const questionId = String(input.question_id);
    const params = new URLSearchParams({
        course_slug: String(input.course_slug),
        lesson_slug: String(input.lesson_slug),
    });
    const result = await callLessonMountApi("DELETE", `/api/questions/${questionId}/lessons?${params}`);
    return JSON.stringify(result, null, 2);
}
async function handleListLessonQuestions(input) {
    const courseSlug = String(input.course_slug);
    const lessonSlug = String(input.lesson_slug);
    const result = await callLessonMountApi("GET", `/api/lessons/${courseSlug}/${lessonSlug}/questions`);
    return JSON.stringify(result, null, 2);
}
async function handleReorderLessonQuestions(input) {
    const courseSlug = String(input.course_slug);
    const lessonSlug = String(input.lesson_slug);
    const result = await callLessonMountApi("PUT", `/api/lessons/${courseSlug}/${lessonSlug}/questions`, { question_ids: input.question_ids });
    return JSON.stringify(result, null, 2);
}
