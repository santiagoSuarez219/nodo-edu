...
export const tools: Tool[] = [
  {
    ...
    description:
      "Lista los cursos académicos del docente. Úsala para obtener los IDs de los cursos y evitar tener que adivinarlos.",
    inputSchema: {
      type: "object",
      properties: {},
      required: [],
  ...
  },
  {
    ...
    description:
      "Lista evaluaciones del docente, filtrable por curso y estado de publicación.",
    inputSchema: {
      type: "object",
      properties: {
        academic_course_id: {
  ...
  },
  {
    ...
    description:
      "Obtiene el detalle completo de una evaluación: configuración, las 3 variantes con sus preguntas, puntos y puntuación total.",
    inputSchema: {
      type: "object",
      properties: {
        groupId: {
  ...
  },
  {
    ...
    description:
      "Crea una evaluación completa en una sola operación atómica. Si falla algo, no se crea nada. Proporciona la configuración compartida y un array de variantes (cada una con sus preguntas).",
    inputSchema: {
      type: "object",
      properties: {
        academic_course_id: {
  ...
  },
  {
    ...
    description:
      "Actualiza la configuración compartida de una evaluación (no las variantes ni preguntas).",
    inputSchema: {
      type: "object",
      properties: {
        groupId: {
  ...
  },
  {
    ...
    description:
      "Reemplaza el conjunto completo de preguntas de una variante específica sin afectar las otras.",
    inputSchema: {
      type: "object",
      properties: {
        groupId: {
  ...
  },
  {
    ...
    description:
      "Elimina una evaluación. Falla con 409 si ya tiene respuestas de estudiantes.",
    inputSchema: {
      type: "object",
      properties: {
        groupId: {
  ...
  },
  {
    ...
    description:
      "Publica una evaluación tras validar invariantes: mínimo 2 variantes, ninguna vacía, puntuación total igual en todas.",
    inputSchema: {
      type: "object",
      properties: {
        groupId: {
  ...
  },
  {
    ...
    description:
      "Obtiene el reparto de variantes por estudiante (solo lectura): qué variante le tocó a cada matriculado.",
    inputSchema: {
      type: "object",
      properties: {
        groupId: {
...
async function handleListAssignmentGroups(input: Record<string, unknown>): Promise<string> {
...
async function handleUpdateAssignmentGroup(input: Record<string, unknown>): Promise<string> {
  const groupId = String(input.groupId);
...
async function handleReplaceVariantQuestions(
...
async function handlePublishAssignmentGroup(input: Record<string, unknown>): Promise<string> {
  const groupId = String(input.groupId);
  const result = await callAssignmentApi(
    "POST",
...
async function handleGetVariantAllocations(input: Record<string, unknown>): Promise<string> {
  const groupId = String(input.groupId);
  const result = await callAssignmentApi(
    "GET",
