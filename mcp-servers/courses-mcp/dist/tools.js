...
export const tools = [
    {
        ...
        description: "Lista todas las lecciones y guías del catálogo de un curso, en orden, indicando cuáles están deshabilitadas para los estudiantes. Úsala **antes** de abrir o cerrar una lección para confirmar el `lesson_slug` exacto y el estado actual.",
        inputSchema: {
            type: "object",
            properties: {
                course_slug: {
    ...
    },
    {
        ...
        description: "Consulta si una lección concreta está disponible para los estudiantes, con la fecha y el motivo por el que se cerró, si aplica.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    },
    {
        ...
        description: "Abre (`enabled: true`) o cierra (`enabled: false`) una lección para los estudiantes. **Afecta a todos los grupos y semestres de ese curso a la vez.** El efecto es inmediato, sin desplegar. Confirma siempre con el docente antes de usarla.",
        inputSchema: {
            type: "object",
            properties: {
                course_slug: { type: "string", description: "Slug del curso." },
...
export async function processToolCall(toolName, toolInput) {
    switch (toolName) {
        ...
        case "set_lesson_availability": {
            ...
            const body = { enabled: toolInput.enabled };
            if (toolInput.reason !== undefined)
                body.reason = toolInput.reason;
            return await callCoursesApi("PATCH", `/${courseSlug}/lessons/${lessonSlug}`, body);
        }
        default:
