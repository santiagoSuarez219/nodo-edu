...
```mermaid
xychart-beta
    title "Operaciones ejecutadas segun el tamano de la entrada n"
    x-axis [10, 100, 1000, 10000]
    y-axis "Operaciones" 0 --> 10000
    line [1, 1, 1, 1]
    line [3, 7, 10, 13]
    line [10, 100, 1000, 10000]
    line [100, 10000, 1000000, 10000000]
```
...
Del arreglo de clientes: guardar `n` clientes siempre cuesta memoria O(n)
—eso no cambia—, pero cuánto tiempo toma *encontrar* uno depende del
algoritmo de búsqueda que elijas sobre ese mismo arreglo.
...
## Reglas para simplificar: constantes y término dominante

Contar operación por operación es tedioso y, además, innecesario: Big O
solo le importa cómo crece el costo cuando `n` es grande, así que existen
tres reglas para simplificar cualquier análisis:

1. **Pasos secuenciales se suman.** Dos bucles uno detrás del otro, cada
   uno O(n), dan O(n) + O(n) — no se multiplican.
2. **Bucles anidados se multiplican.** Un bucle O(n) dentro de otro bucle
   O(n) da O(n) × O(n) = O(n²).
3. **Se descartan constantes y términos de menor orden.** O(2n + 5) se
   simplifica a O(n); O(n² + n) se simplifica a O(n²) — el término que
   domina cuando `n` crece es el único que importa.

```mermaid
flowchart LR
  A["for cliente en clientes:\n  for tx en cliente.transacciones():\n    procesar(tx)"] --> B["Costo parcial:\nbucle externo n clientes\nbucle interno k transacciones c/u"]
  B --> C["n * k operaciones\n(bucles anidados: se multiplican)"]
  C --> D["Se descartan constantes\ny terminos menores"]
  D --> E["O(n * k)\nsi k ~ n, queda O(n^2)"]
```

Recorrer la lista de clientes y, dentro de cada uno, su lista de
transacciones, es exactamente ese patrón: un bucle anidado dentro de otro.
Si el número de transacciones por cliente crece al mismo ritmo que el
número de clientes, el costo total queda en O(n²) — el mismo orden que
`hayTransaccionesDuplicadas` de la sección anterior, aunque el código se
vea distinto.
...
| Lista enlazada         | *pendiente — semanas 5–6* | *pendiente* | *pendiente* | *pendiente* |
| Pila                   | *pendiente — semana 8* | *pendiente* | *pendiente* | *pendiente* |
| Cola                   | *pendiente — semana 9* | *pendiente* | *pendiente* | *pendiente* |
| Árbol binario de búsqueda (BST) | *pendiente — semana 15* | *pendiente* | *pendiente* | *pendiente* |
