...
En Composición, agregación y diagramas de paquetes viste cómo UML anota una
relación de composición entre dos clases con multiplicidades. Ese mismo
diagrama, cuando te lo entregan ya resuelto, se lee de izquierda a derecha
para decidir qué escribir en Java:
...
classDiagram
  ...
  Banco "1" *-- "0..*" CuentaAhorros : contiene
```

La multiplicidad `"1" *-- "0..*"` dice: un `Banco` contiene cero o más
`CuentaAhorros`, y cada `CuentaAhorros` pertenece exactamente a un `Banco`
(composición: si el `Banco` desaparece, sus cuentas desaparecen con él). Esa
lectura se traduce, casi línea por línea, a una colección como campo y a un
método que agrega a esa colección:
...
| `Banco "1" *-- "0..*" Cuenta` | Composición: colección como campo | `List<Operable> cuentas` en `Banco` |
...
- Un diagrama de clases con relaciones (`*--`, multiplicidades) se traduce
