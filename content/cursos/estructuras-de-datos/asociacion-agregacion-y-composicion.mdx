...
**Situación:** en la liga de fútbol, un `Jugador` solo tiene sentido dentro
de su `Equipo` —si el equipo se disuelve, el jugador deja de estar fichado
por él, igual que `Cuenta` y sus `Movimiento`s: es una composición—, pero el
diagrama todavía no dice cuántos jugadores puede tener un equipo, ni si un
equipo puede existir sin ninguno todavía fichado.
