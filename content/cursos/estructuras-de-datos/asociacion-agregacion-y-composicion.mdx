...
**Situación:** en la liga de fútbol, el objeto `Jugador` no representa a la
persona en general, sino su fichaje por un `Equipo` concreto: si el equipo se
disuelve, ese fichaje deja de existir con él, igual que `Cuenta` y sus
`Movimiento`s —es una composición—, pero el diagrama todavía no dice cuántos
jugadores puede tener un equipo, ni si un equipo puede existir sin ninguno
todavía fichado.
