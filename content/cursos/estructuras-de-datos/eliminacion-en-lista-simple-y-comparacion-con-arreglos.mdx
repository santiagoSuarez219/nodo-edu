...
Las tres eliminaciones señalan ese caso borde con `NoSuchElementException`,
que a diferencia de `IndexOutOfBoundsException` no vive en `java.lang`:
necesita `import java.util.NoSuchElementException;` al inicio de
`ListaSimple.java`.
...
flowchart TB
  ...
  subgraph ListaEliminar["Lista: eliminar en medio"]
    direction LR
    LA["Nodo Tx1"] -.->|"referencia rota"| LB["Nodo Tx2"]
    LA -->|"siguiente reescrito"| LC["Nodo Tx3"]
    LC --> LD["Nodo Tx4"]
  end
```
...
el ciclo entero de operaciones sobre un solo sentido de recorrido: desde el
nodo y la memoria dinámica hasta el reenlace de referencias que reemplaza
todo desplazamiento.
