...
Antes de que empieces, el profesor modela en vivo, una por una, las clases
que faltan del Sistema Bancario — el proyecto de referencia del curso.
Escríbelas tú también en tu propio computador mientras el profesor las
construye en el tablero; el objetivo es que veas el proceso completo, en
vivo, antes de repetirlo por tu cuenta con tu propio proyecto.
...
Si tu equipo tiene 3 integrantes y el proyecto tiene 4 entidades, repártanse
así: cada uno toma una, y la cuarta queda para quien termine primero o se
divide entre dos si prefieren trabajar en pareja sobre esa clase. Pónganse
de acuerdo **antes** de empezar a escribir código, para no duplicar trabajo.

```mermaid
flowchart LR
  Equipo["Tu equipo\n(3-4 integrantes)"] --> E1["Estudiante 1\nmodela una entidad"]
  Equipo --> E2["Estudiante 2\nmodela otra entidad"]
  Equipo --> E3["Estudiante 3\nmodela otra entidad"]
  Equipo --> E4["Entidad restante:\nse reparte o se hace en pareja"]
```
...
proyecto de aula tiene cuatro entidades candidatas para repartir entre el
...
Entidades que modela el profesor, una por una, en vivo: `Cliente`, `Cuenta`,
`CuentaAhorros`, `CuentaCorriente` y `Transaccion` — hoy, sin herencia
todavía, cada una como una clase independiente con sus propios atributos.
...
Entidades para repartir: `Producto`, `Venta`, `Proveedor`, `Pedido`.
...
Entidades para repartir: `Paciente`, `Medico`, `Cita`, `Consulta`.
...
Entidades para repartir: `Animal`, `Dueño`, `Veterinario`, `Consulta`.
...
Entidades para repartir: `Estudiante`, `Profesor`, `Materia`, `Matricula`.
...
Entidades para repartir: `Jugador`, `Equipo`, `Partido`, `Gol`.
