...
> **`if __name__ == "__main__":`** protege el punto de entrada de un script: el código dentro de ese bloque (normalmente una llamada a `main()`) solo se ejecuta cuando el archivo corre directamente, no cuando otro módulo lo importa. Es la convención estándar para separar "definir funciones" de "ejecutar el programa".
