...
La misma idea aparece en problemas que no tienen nada que ver con ordenar nombres:

<Callout tipo="nota">
**Programar los horarios de una universidad** —asignar miles de grupos a cientos de docentes y salones sin que dos choquen en el mismo bloque— es, en el fondo, un problema de asignación con restricciones. "Probar combinaciones hasta que una funcione" no escala igual que una estrategia que descarta de entrada ramas completas de opciones inválidas.

**Calcular la ruta más rápida para una ambulancia** sobre la red vial de una ciudad es un problema de caminos mínimos sobre un grafo. Recorrer todas las rutas posibles no escala igual que un algoritmo que aprovecha la estructura de la red para descartar caminos que nunca podrían ser los más cortos.

</Callout>

Se repite el mismo argumento en los dos: una organización real depende de que un algoritmo termine a tiempo, y la estrategia que elige para resolver el problema —no el hardware donde corre— es lo que decide si eso ocurre.
...
Esa diferencia —del orden de lo que consume un hogar entero en un año— no la produjo un servidor más moderno ni una planta de energía nueva. La produjo cambiar el algoritmo. Cuando el proceso corre cada noche, durante años, la decisión de diseño que tomaste en una tarde se convierte en una cifra ambiental.
...
stateDiagram-v2
  [*] --> Inicializacion
  Inicializacion --> Mantenimiento: "la invariante es cierta\nantes de la primera iteracion"
  Mantenimiento --> Mantenimiento: "cada iteracion preserva\nla invariante"
  Mantenimiento --> Terminacion: "el ciclo termina"
  Terminacion --> [*]: "la invariante + la condicion\nde salida implican correccion"
```
...
Este argumento no depende de los valores concretos de la traza anterior: es válido para cualquier arreglo de cualquier tamaño, incluidos los 30.000 apellidos. Es el mismo principio que usarás para probar la corrección de cada algoritmo nuevo que veas en el curso — inducción matemática aplicada a un programa.
