...
| Sprint del proyecto | Ninguno todavía. El proyecto integrador arranca en la Semana 12; hoy se trabaja sobre datasets de juguete provistos por el docente (tienda, temperaturas, factura, calificaciones, ferretería) |
...
- Aplicar operadores aritméticos (`+`, `-`, `*`, `/`, `//`, `%`, `**`), de comparación (`==`, `!=`, `<`, `>`, `<=`, `>=`) y lógicos (`and`, `or`, `not`) sobre datos de distintos datasets de juguete.
...
el dataset de la tienda y luego el grupo lo repite de inmediato, avanzando
por el taller de diez ejercicios como un único hilo de trabajo.
...
| 10–35 min | Demo guiada: variables y tipos (Ejercicio 1) | Proyecta Colab y resuelve en vivo el Ejercicio 1 del taller (inventario de la tienda), usando `type()` en cada variable | Replica cada celda en su propio notebook, línea por línea, sin adelantarse |
| 35–40 min | Punto de control 1 | Recorre el salón revisando pantallas | Tiene las siete variables del Ejercicio 1 asignadas con el tipo correcto y al menos dos `type()` ejecutados |
| 40–65 min | Ejercicios 2 y 3 — aritmética sobre la tienda | Presenta los enunciados; circula resolviendo dudas | Resuelve el total de la compra con descuento (Ejercicio 2) y el reparto en paquetes (Ejercicio 3) |
| 65–70 min | Punto de control 2 | Revisa avances tras el Ejercicio 3; identifica quién va atrasado | Reporta su avance si el docente pregunta |
| 70–100 min | Ejercicios 4 a 7 — comparación, temperaturas, lógicos y factura | Presenta los enunciados en orden; aplica diferenciación (ver más abajo) según el punto de control anterior | Resuelve comparar precios de café (4), temperaturas (5 y 6) y la factura de servicios (7) |
| 100–105 min | Punto de control 3 | Revisa avances tras el Ejercicio 7; confirma que las tres categorías de operadores ya tienen ejemplos resueltos | Reporta su avance si el docente pregunta |
| 105–115 min | Ejercicios 8 a 10 — calificaciones, asistencia e intercambio | Presenta los enunciados finales; ofrece las Extensiones Sugeridas (Bonus) a quien ya terminó | Resuelve promedio y aprobación (8), asistencia (9) e intercambio de variables (10); quien termina antes explora el Bonus |
| 115–120 min | Cierre y subida con Flujo A | Recuerda el nombre exacto del archivo y la carpeta destino (`ejercicios/`); resuelve dudas de último minuto sobre el botón de GitHub | Sube su notebook a `ejercicios/` con `Archivo → Guardar una copia en GitHub` y confirma en GitHub que quedó visible |

Suma: 120 minutos.

## Desarrollo paso a paso

Solución de referencia completa de los diez ejercicios del taller, en el
mismo orden que la guía del estudiante. Cada bloque puede resolverse en una
única celda de código o en varias — lo importante es que cada ejercicio
quede precedido por su celda de texto, como se le pide al estudiante.

### Ejercicio 1 — Inventario de la tienda (demo guiada, proyectada)
...
precio_lapiz = 800                # int
precio_cafe = 6200.50             # float
hay_descuento = False             # bool
cantidad_cuadernos = 3            # int
cantidad_lapices = 5              # int
...
print("Total cuadernos:", total_cuadernos, "- Total lápices:", total_lapices)

total_general = total_cuadernos + total_lapices + precio_cafe
print("Total general de la compra:", total_general)

descuento = total_general * 0.10        # 10% de descuento
total_con_descuento = total_general - descuento
print("Total con descuento:", total_con_descuento)
```

Resultados esperados clave: `total_general = 21200.5`;
`total_con_descuento = 19080.45`.
...
Resultados esperados clave: `paquetes_completos = 0`, `cuadernos_sueltos = 3`
(porque `3 // 4 == 0` y `3 % 4 == 3`).
...
promedio_tres_dias = (temp_lunes + temp_martes + temp_miercoles) / 3
...
Resultado esperado clave: `promedio_tres_dias ≈ 19.7667`.

### Ejercicio 6 — Temperatura templada y día caluroso

```python
dia_templado = (temp_lunes >= 15) and (temp_lunes <= 25)
print("¿El lunes tuvo una temperatura templada (entre 15 y 25)?", dia_templado)   # True

print("¿Al menos un día superó los 20 grados?", (temp_lunes > 20) or (temp_martes > 20) or (temp_miercoles > 20))   # True
```
...
```python
# Reutiliza "promedio" del Ejercicio 8
...
Este ejercicio sigue inspirado en la pregunta del banco
`ba4a3294-5052-49aa-bd97-52690f5ac3bd` sobre intercambio de variables con
variable temporal.

Resultados clave para verificar de un vistazo en pantalla, en orden:
Ejercicio 2 → `21200.5` / `19080.45`; Ejercicio 3 → `0` paquetes, `3`
sueltos; Ejercicio 5 → promedio `≈ 19.7667`; Ejercicio 7 → `88537.0`;
Ejercicio 8 → promedio `≈ 4.1667`; Ejercicio 10 → valores invertidos
(`180` / `250`).

### Cierre: subir con Flujo A
...
3. Ruta: `ejercicios/sesion-02-taller-variables-tipos-operadores.ipynb`.
4. Mensaje: `Taller de variables, tipos y operadores`.
...
|---|---|---|
| Minuto ~35 | Notebook tras la demo guiada (Ejercicio 1) | Tiene las siete variables del Ejercicio 1 asignadas con los tipos correctos y al menos dos `type()` ejecutados con salida visible |
| Minuto ~70 | Avance tras los Ejercicios 2 y 3 | El total de la compra con descuento y el reparto en paquetes están resueltos y ejecutados sin error |
| Minuto ~105 | Avance tras el Ejercicio 7 | Las tres categorías de operadores (aritméticos, comparación, lógicos) tienen al menos un ejemplo resuelto en los Ejercicios 4 a 7 |
| Minuto ~115 | Notebook completo, antes de subir | Los diez ejercicios ejecutados de arriba a abajo sin error (`Entorno de ejecución → Ejecutar todas`); hay una celda de texto por ejercicio |
| Minuto ~120 | Repositorio en GitHub tras el push | `ejercicios/sesion-02-taller-variables-tipos-operadores.ipynb` (o el nombre que se haya acordado) aparece listado |
...
| `TypeError: can only concatenate str (not "int") to str` al construir un mensaje con `print` | Intentó unir un `str` y un `int` con `+` directamente, ej. `"Total: " + total_general` | Mostrar `print("Total:", total_general)` (con coma) o convertir con `str(total_general)` |
| `cantidad_cuadernos // 4` da un resultado con decimales inesperados | Usó `/` en vez de `//` pensando que ambas son "división" | Contrastar `7 / 2` (`3.5`) contra `7 // 2` (`3`) en una celda aparte, en vivo |
...
| El notebook tiene celdas de código sin ninguna celda de texto | Se saltó el requisito de documentar cada ejercicio, por apuro | Señalar el criterio de la rúbrica correspondiente antes de que seleccione entregar |
| En el Ejercicio 10, después de "intercambiar" ambas variables quedan con el mismo valor | Sobrescribió `precio_tornillo` antes de guardar su valor original en `temporal`, perdiendo el dato de partida | Pedir que impriman el valor de `temporal` justo después de asignarlo, antes de seguir, para confirmar que guardó el valor correcto |
...
- *"En el Ejercicio 10, ¿por qué no basta con escribir `precio_tornillo =
  precio_tuerca` seguido de `precio_tuerca = precio_tornillo`?"* — Respuesta
  esperada: porque la primera línea ya sobrescribió `precio_tornillo`, así
  que la segunda línea copia el valor nuevo (no el original) en
  `precio_tuerca`; por eso hace falta guardar uno de los dos valores en una
  variable temporal antes de sobrescribir nada.

## Diferenciación

**Estudiantes de primer semestre que no arrancan (andamiaje mínimo aceptable):**
- Entregarles una plantilla con las variables del Ejercicio 1 **ya
...
- Aceptar como avance mínimo válido: los Ejercicios 1 a 5 resueltos
  correctamente (variables, aritmética y al menos una comparación) — no
  exigir el taller completo si el tiempo no alcanza.
- Emparejarlos con un estudiante avanzado desde el bloque de los Ejercicios
  2 y 3 (minuto ~40), no esperar a los ejercicios de temperaturas.
...
- Ofrecerles las Extensiones Sugeridas (Bonus) de la guía del estudiante
  (tiempo de un viaje con `/`, tramos de un viaje con `or`, `%` con números
  negativos, o explicar el orden de evaluación de una expresión combinada)
  apenas terminen el Ejercicio 10, sin esperar al bloque 105–115 min si van
  más rápido.
- Pedirles que, en una celda de texto, expliquen con sus palabras por qué
  `float * int` da `float` (visible en el Ejercicio 7, `consumo_kwh` entero
  por `tarifa_kwh` decimal) — sirve como verificación de comprensión, no
  solo de ejecución correcta.
...
- Confirmar que cada estudiante subió su notebook único del taller a
  `ejercicios/` con Flujo A — es la evidencia de seguimiento de hoy.
...
- Los cinco datasets del taller, redactados exactamente como aparecen en
  esta guía y en la guía del estudiante, para que los valores y resultados
  esperados coincidan entre docente y estudiantes: inventario de la tienda
  (cuaderno, lápiz, café), temperaturas diarias, factura de servicios
  (consumo y tarifa de kWh), calificaciones (tres notas parciales) e
  inventario de la ferretería (tornillo y tuerca).
- Un notebook propio ya preparado con el Ejercicio 1 (demo guiada), para
