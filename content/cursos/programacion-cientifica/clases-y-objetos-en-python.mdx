...
> **Docstring:** frase entre triples comillas que va en la primera línea del cuerpo de una función, una clase o un método y explica qué hace. Es la documentación que acompaña al código, como la línea `"""Una lectura de PM2.5 de una estacion."""` de arriba.

Fíjate también en el nombre: las clases se escriben en `PascalCase` (`Medicion`, `Estacion`), con cada palabra en mayúscula inicial y sin guiones bajos, mientras que las variables y las funciones siguen en `snake_case`.
>
...
Esta forma de organizar datos con su comportamiento es la que sostiene a las librerías científicas que vienen: un arreglo de NumPy o una tabla de Pandas son objetos, y los métodos que les aplicarás (como calcular un promedio) se invocan de la misma forma que `estacion.promedio()`.
