...
Para saber cuántas unidades hay de `"Taladro"`, buscas su posición en la primera lista (índice `2`) y usas ese mismo índice en la segunda. Funciona, hasta que llega una herramienta nueva:

```python
nombres_herramientas.append("Llave inglesa")
# se te olvidó esta línea:
# cantidades_herramientas.append(8)
```

Ahora `nombres_herramientas` tiene 4 elementos y `cantidades_herramientas` sigue con 3. El índice `3` de nombres apunta a `"Llave inglesa"`, pero ese mismo índice ya no existe en cantidades — y si existiera, apuntaría a un número que no le corresponde. Nada en Python te avisa de este error: el programa sigue corriendo con datos desalineados.
...
```mermaid
flowchart LR
  I["inventario"] --> D0["0: {nombre: Martillo, precio: 15990.0, cantidad: 12}"]
  I --> D1["1: {nombre: Taladro, precio: 189900.0, cantidad: 4}"]
  I --> D2["2: {nombre: Serrucho, precio: 25000.0, cantidad: 9}"]
```
...
En el taller de condicionales y bucles construiste listas nuevas recorriendo una lista existente con `for` y acumulando en `append`:
