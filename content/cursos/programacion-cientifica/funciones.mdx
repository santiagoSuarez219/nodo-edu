...
# celda 2: convierte las lecturas de otro sensor de campo (copiado y pegado)
lecturas_ug_campo = []
...
`valor` y `unidad` son los **parámetros**: los datos que la función necesita para trabajar. `unidad="mg/m3"` es un **valor por defecto** — si no lo indicas al llamar la función, asume que la lectura viene del sensor de campo (el caso más común en la red) y la convierte. `return` entrega el resultado de vuelta a quien llamó la función, para que puedas guardarlo en una variable o usarlo en la siguiente línea.
...
> **Función:** bloque de código con nombre que recibe parámetros, ejecuta un cuerpo, y opcionalmente `return`a un valor. Se define una vez con `def` y se reutiliza tantas veces como haga falta, con datos distintos cada vez.
...
`filter` resuelve el problema complementario: quedarte solo con las lecturas que cumplen una condición. Por ejemplo, las que superan la categoría Moderada (más de 35.4 µg/m³):
...
- **Prueba tu función con un caso simple antes de aplicarla a todo el dataset.** `convertir_a_microgramos(1)` debería dar `1000.0` — verificarlo con un valor conocido cuesta una línea y evita arrastrar un error a los cientos de lecturas de la red.
